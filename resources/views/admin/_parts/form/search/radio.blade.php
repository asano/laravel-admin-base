    ...
    <div class="col-md-10">
        @foreach ($_configs as $_val => $_config)
            <div class="custom-control custom-radio custom-control-inline{{ $errors->has($_name) ? ' is-invalid': '' }}">
                <input type="radio" id="{{ $_id }}_{{ $_config[$_value_key] }}" name="{{ $_name }}" class="custom-control-input {{ $errors->has($_name) ? ' is-invalid': '' }}" value="{{ $_config[$_value_key] }}" {{ ($_input[$_name] ?? null) == $_config[$_value_key] ? ' checked' : '' }}>
